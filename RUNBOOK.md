...
1. **Ollama** — auto-detected at `http://localhost:11434`. Ollama must allow the Pages origin: set `OLLAMA_ORIGINS=https://tbalt88.github.io` (Windows: env var; Mac: `launchctl setenv OLLAMA_ORIGINS https://tbalt88.github.io` — session-only) and restart Ollama. Pick a small model for *shortlist* and the best you can run for *reasoning* (on a 4 GB GPU use the smallest for both).
   ...
   - **Restart without losing the env:** `launchctl kickstart -k gui/$(id -u)/homebrew.mxcl.ollama` (NOT `brew services restart`, which regenerates the plist). After `brew upgrade ollama` that kickstart is all you need.
   - **"Detect" green but every ask falls back to lexical rank:** Detect only calls `/api/tags`; a broken install can list models yet fail to load one. Signature (Mac, 2026-08-18, brew 0.30.7): Ollama log / browser console `Ollama 500: error starting llama-server: llama-server binary not found`. Fix was `brew upgrade ollama` (runners under `libexec/lib/ollama/` appeared) + kickstart. Quick check: `ollama run llama3 "say OK"` from a terminal — if that fails, the page can't work either.
